...
class Button:
    def __init__(self, surface, on_click_func, color, x, y, width, height, text_color=BLACK, text='', text_size=20, id=None):
        self.id = id
        self.color = color
...
class Menu:
    def __init__(self, surface, menu_height, menu_width):
    ...
    def add_button(self, color, on_click_function=None, text='', text_color=BLACK, text_size=20):
        x, y = self.width * 0.1, len(self.buttons) * (self.height * 0.15) + 10
        width, height = self.width * 0.8, self.height * 0.1
        if on_click_function is None:
            on_click_function = lambda: None
